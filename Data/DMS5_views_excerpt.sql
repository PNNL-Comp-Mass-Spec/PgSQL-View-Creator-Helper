...
CREATE VIEW "public"."v_organism_export"
AS
...
;
CREATE VIEW "public"."v_eus_export_dataset_metadata"
AS
SELECT D.Dataset_ID AS Dataset_ID,
       ...
       COALESCE(D.Acq_Time_Start, D.DS_created) AS Dataset_Acq_Time_Start,
       ...
       COALESCE(U_ProjMgr.U_Name, C.CM_Proj_Mgr_PRN) AS Project_Manager,
       COALESCE(U_PI.U_Name, C.CM_PI_PRN) AS Project_PI,
       COALESCE(U_TechLead.U_Name, C.CM_Technical_Lead) AS Project_Technical_Lead,
       ...
       RR.RDS_EUS_Proposal_ID AS EUS_Proposal,APath.AP_archive_path||'/'||D.DS_folder_name AS Dataset_Path_Aurora
	FROM t_campaign C
